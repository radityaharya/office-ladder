...
- **Gameplay is a real but minimal slice, not the full designed ruleset.** The engine's `turn.roll` transition does: roll → move → receptionist-pass salary → **auto-attempt promotion** (if the player's money/reputation meet the next rank's cost from `packages/content/src/deadline-dash/ranks.ts`, silently promote, no player choice) → **win check** (reaching `rank.director`, i.e. `MatchEndReason: "director-reached"`, ends the match and sets `GameState.outcome.winnerPlayerIds`). This makes a room genuinely completable and winnable end-to-end.
  - **Not implemented**: event/management card draws, tile-specific effects beyond salary (the ~20 tile kinds in `board.ts` each carry an `effects: EffectDescriptor[]` array — e.g. `drawCards`, `rollCheck`, `applyStatus`, `auditConfinement` — that the engine's execution layer does not interpret), prompts/decisions (`prompt.respond`, `reaction.play`, etc. — defined as command types in `commands/index.ts` but `legal-actions.ts` only ever enumerates `game.start` and `turn.roll`), hidden character roles/abilities (characters are assigned and shown, but `character.passive` abilities beyond a salary multiplier aren't applied), audits, skip-turns, work-counter milestones.
  - If picking this up: the natural next increment is a generic effect interpreter consuming `BoardTile.effects` (content already carries the data — see `packages/content/src/schema/effects.ts` for the vocabulary), rather than hand-rolling more one-off cases into `roll-turn.ts` the way promotion was added.
- **No new automated test for the promotion/win-condition logic added this session** — `packages/engine/tests/*` still all pass (83/83, unchanged), but there's no dedicated test asserting `resolvePromotion` or the `MatchEnded`/`director-reached` path. Worth adding before extending it further.
- **Combined dev-server boot (`bun run dev`, both processes together) was not verified interactively this session** — the sandboxed environment used to build this couldn't reliably run two concurrent long-lived background processes (repeated tool-level failures unrelated to the code). Each half **was** independently verified working standalone:
  - `apps/server` alone: booted, Better Auth sign-up/sign-in worked against the real Supabase Postgres, room creation worked end-to-end through the full stack (contracts → Hono route → room service → engine).
  - `apps/web` alone (before the proxy rework): Vite booted and served the SPA shell (`GET /` → 200).
  - The proxy config itself (`apps/web/vite.config.ts`'s `server.proxy`) is standard Vite and wasn't smoke-tested combined — do this first if something's not working after a fresh clone.
- **No manual browser playthrough** (create room → join → start → roll → promote → win) was done this session — no browser available in this environment. Do this before considering the game "done."
