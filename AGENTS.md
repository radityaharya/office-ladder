...
2. Author real management-deck card content so `drawCards` pulls from it instead of the synthesized flavor table — this is now the single biggest gap between what the content pack's effect vocabulary implies and what the engine actually does (every other effect type in `packages/content/src/schema/effects.ts` is now interpreted).
...
5. If room volume ever justifies it, migrate `PostgresRoomRepository` off the single-jsonb-blob approach onto the schema's actual normalized/event-sourced tables — `game_events`/`command_receipts`/`game_outbox` in particular would enable real audit trails and reliable cross-instance realtime fan-out, neither of which the current snapshot-only approach gives you.
