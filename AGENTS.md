...
- **Engine tests added for the new logic**: `packages/engine/tests/tile-effects-and-promotion.test.ts` (6 tests) covers a `payResource` deduction, a `restoreResourceToMaximum` energy refill, a full promotion-to-Director win, the "can't afford it, nothing happens" case, and the Workaholic passive applying/not-applying based on tile kind. Full engine suite: 89/89.
